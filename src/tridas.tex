...
\begin{comment}
Sono state svolte diverse fasi di test per verificare la capacità di TriDAS di gestire flussi di dati degli esperimenti ai Jefferson Lab e per confrontare l'efficienza dell'approccio \emph{triggerless} con quello \emph{triggered} \cite{3x3telescope}. In particolare, si illustra l'esperimento per il quale è stato sviluppato l'algoritmo di cui nel prossimo capitolo.
Un prototipo formato da una matrice $3 \times 3$ di cristalli $PbWO_4$ è stato installato nella \emph{Hall-D} dei Jefferson Lab, all'uscita del fascio secondario di elettroni prodotto dal \emph{Pair Spectrometer} (\autoref{fig:3x3} e \autoref{fig:3x3_scheme}). Fasci secondari di elettroni e positroni sono prodotti dall'interazione del fascio primario di fotoni con un convertitore in berillio di spessore \SI{750}{\micro\m} e separati da un campo magnetico di \SI{1.5}{\tesla}. 
Ciascun cristallo ($\SI{2.05}{\cm} \times \SI{2.05}{\cm} \times \SI{20}{\cm}$) è stato collegato a un fotomoltiplicatore di \SI{19}{\mm} di diametro. 
Il prototipo è stato posizionato in modo che il centro del fascio elettronico illumini il cristallo centrale della matrice, con inclinazione perpendicolare alle facce dei cristalli.   
Il test è stato svolto durante la presa dati dell'esperimento \emph{GlueX} con fascio di fotoni da \SI{350}{\nano\ampere} e un fascio secondario di elettroni da \SI{4.7}{\GeV}.
Quindi sono stati utilizzati due diversi sistemi di acquisizione: \emph{triggered} e \emph{triggerless}. Per quest'ultimo, il segnale di ciascun PMT è stato digitalizzato e inviato a TriDAS, settando una soglia di $\small \sim$\SI{2}{\GeV} per gli eventi L1.
\end{comment}
